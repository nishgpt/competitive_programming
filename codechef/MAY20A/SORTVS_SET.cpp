...
Value call(int n, string key, int match, unordered_map<int, bool> mp) {
	...
	unordered_map<int, bool> mpcopy;
	while(it != mp.end()) {
		mpcopy = mp;
		mpcopy.erase(it->first);
		int i = it->first;
		for(int j=1;j<=n;j++) {
			if(v[j] == visited[i] && v[j] != visited[j]) {
				...
				if(dp[key1].value == -1) ans = min(ans, 1 + call(n, key1, match + inc, mpcopy).value);
				else {
					ans = min(ans, 1 + dp[key1].value);
				}
				
				if(inc > 1) mpcopy[j] = true;
				temp = v[j], v[j] = v[i], v[i] = temp;
				x = key1[i-1], key1[i-1] = key1[j-1], key1[j-1] = x;
			}
		}
		...
		//~if(v[i] != visited[i]) {
			//~for(int j=1;j<=n;j++) {
				//~if(v[j] == visited[i] && v[j] != visited[j]) {
					...
					//~int inc = 1;
					//~if(v[j] == visited[j]) inc++;
					
					//~if(dp[key1].value == -1) ans = min(ans, 1 + call(n, key1, match + inc).value);
					//~else {
						//~ans = min(ans, 1 + dp[key1].value);
					//~}
...
int main() {
	...
	while(t--) {
		...
		string key;
		int match  = 0;
		unordered_map<int, bool> non_matching;
		for(int i=1;i<=n;i++) {
			if(v[i] == visited[i]) match++;
			else non_matching[v[i]] = true;
			key = key + (char) (v[i]+'a');
		}
		
		key_hits = 0, func_calls = 0;
		cout<<call(n, key, match, non_matching).value<<endl;
